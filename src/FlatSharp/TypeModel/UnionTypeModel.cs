    ...
    public override CodeGeneratedMethod CreateSerializeMethodBody(SerializationCodeGenContext context)
    {
        ...
        for (int i = 0; i < this.UnionElementTypeModel.Length; ++i)
        {
            ...
            if (elementModel.SerializesInline)
            {
                ...
                inlineAdjustment =$@"
                    var offset = context.{nameof(SerializationContext.AllocateSpace)}({elementModel.PhysicalLayout.Single().InlineSize}, {elementModel.PhysicalLayout.Single().Alignment});
                    ";
            }
            else
            {
...
$@"
        ...
        string serializeBlock = $@"
            ...
            return (discriminatorValue, {context.OffsetVariableName}.offset1);
            ";
